...
If you've played with service worker without reading too much about it, it's likely that the lifecycle has hit you in the face like a brick when you least expect it. And let's face it, the time you least expect to be hit by a brick is *all of the time*.

So to avoid those bricks, here's a guide to the service worker lifecycle. It's pretty exhaustive (in that it's fully detailed and also exhauting), but I'll add some quick notes to the top of each section.
...
* Ensure a page in a scope is controlled by the same service worker (or no service worker) throughout.
* Ensure there's only one version of your site running at once.

That last one is pretty important. Without service worker, users can load one tab to your site, then later open another. This can result in two versions of your site running at the same time. Sometimes this is ok, but if you're dealing with storage you can easily end up with two tabs having very different opinions on how their shared storage should be managed. This can result in errors, or worse, data loss.
...
* A service worker won't receive events like "fetch" & "push" until it successfully finishes installing.
...
It registers a service worker, and makes an image of a dog appear after 3 seconds.
...
It caches an image of a cat, and serves it whenever there's a request for `/dog.svg`, because cats are better (*they just are*). However, if you [run the above](https://cdn.rawgit.com/jakearchibald/80368b84ac1ae8e229fc90b3fe826301/raw/ad55049bee9b11d47f1f7d19a73bf3306d156f43/){: .external}, you'll see a dog the first time.

### Scope & control

The default scope of your service worker registration is `./` relative to the script URL. This means if you register a service worker at `//example.com/foo/bar.js` it has a default scope of `//example.com/foo/`.

We call pages, workers, and shared workers "clients". Your service worker can only control clients that are in-scope. Once a client is "controlled", its fetches go through a service worker. You can detect if a client is controlled via `navigator.serviceWorker.controller` which will be null or a service worker instance.
...
Chrome's devtools will show the error in the service worker section of the application tab:
...
The first event a service worker gets is "install". It's triggered as soon as the worker initially executes, and it's only called once per service worker. If you alter your service worker script it's considered to be a different service worker, and it'll get its own "install" event - we'll cover updates in detail later.
...
* An update is triggered:
    ...
    * On functional events "push" & "sync", unless there's been an update check within the previous 24 hours.
    * On calling `.register` *only if* the service worker URL has changed.
* Your caching headers are respected, although capped to 24 hours, when fetching the service worker for updates (we're going to make this opt-in behaviour, as it catches people out). You probably want a max-age of 0 on your service worker script.
...
* The updated service worker is launched alongside the existing one, and gets its own "install".
* If your new worker has a non-ok status code (eg 404), fails to parse, throws an error during execution, or rejects during install, the new worker if thrown away, but the current one will remain in operation.
* Once successfully installed, the updated worker will delay activating until the existing worker is controlling zero clients.
* `self.skipWaiting()` prevents this delay, and activates as soon as it's finished installing.
...
[Check out a demo of the above](https://cdn.rawgit.com/jakearchibald/80368b84ac1ae8e229fc90b3fe826301/raw/ad55049bee9b11d47f1f7d19a73bf3306d156f43/index-v2.html){: .external}, you should still see an image of a cat. Let's dig into why…
...
However, for [the demo above](https://cdn.rawgit.com/jakearchibald/80368b84ac1ae8e229fc90b3fe826301/raw/ad55049bee9b11d47f1f7d19a73bf3306d156f43/index-v2.html){: .external}, I *have* changed the URL of the service worker. This is so, for the sake of the demo, you can switch between the versions easily. It isn't something I'd do in production.

### Update

In addition to the above, you can also trigger updates yourself from the registration object.
...
If you expect the user to be using your site for a long time without reload, you may want to call update on an interval (such as hourly). 

The spec is designed so browsers can trigger updates whenever they want. Eg in future browsers may perform daily overnight updates to service workers for all sites you've added to your homescreen.
...
Even if you only have one tab open to the demo, refreshing the page isn't enough to let the new version take over. This is due to how browser navigations work. When you navigate, the current page doesn't go away until the response has been received, and even then the current page may stay if the response has a `Content-Disposition` header. Due to this overlap, the current service worker is always controlling clients.

To get the update, close or navigate-away all tabs using the current service worker. Then, when you [navigate to the demo](https://cdn.rawgit.com/jakearchibald/80368b84ac1ae8e229fc90b3fe826301/raw/ad55049bee9b11d47f1f7d19a73bf3306d156f43/index-v2.html){: .external} again, you should see the horse. This is a pain during development, but devtools has ways to make it easier, which I'll cover later.

### Activate

This fires once the old service worker is gone, and your new service worker is available to control clients. This is the ideal time to do stuff that you couldn't do while the old worker was still in use, such as migrating databases and clearing caches.
...
Note that this means your new service worker is likely controlling pages that were loaded with some other version. This means some of your page's fetches will have been handled by your old service worker, but your new service worker will be handling subsiquent fetches. If this might break things, don't use `skipWaiting()`.
...
1. Install it as a new version even if it's byte-identical, meaning your install event runs, updating your caches.
...
The whole update cycle is observable:
...
Bearing that in mind, here are a few patterns I've dreamt up, your dreams may be better.

### Offer updates to the user

Track updates as above, and when a new version becomes waiting, ask the user if they want to update. If the user clicks "update", `skipWaiting`, and refresh the page once the controller changes. [SVGOMG](https://jakearchibald.github.io/svgomg/){: .external} uses this pattern.

### Tell the user what's in the new version before they update

Give each service worker a "version number", eg "1.0.1", and store it in IndexedDB ([idb-keyval](https://www.npmjs.com/package/idb-keyval){: .external} can make this easier). If you store the installing version and active version numbers seperately, you have what you need to fetch a changelog between the two and tell the user what's in the update.

### Take drastic action for 'urgent' updates

Once you have the version you're on, and the version for the update, you can ask the server if the update fixes any hideous security issues. If you, you may choose to `skipWaiting` and refresh all open pages instantly. It isn't a great user experience, but probably better than leaving the user running exploitable code.

### Generate cache versions automatically

If you don't need a specific versioning scheme, you could generate one automatically. In [my blog's service worker](https://jakearchibald.com/sw.js){: .external} the static cache name is a hash of the JS, CSS, and page components. This means the service worker automatically updates when I make changes.